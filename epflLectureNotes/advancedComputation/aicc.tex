...
\subsubsection{Some wrong proofs}
...
$$ S(k+1) = \min_{1\leq l \leq k}(l(k+1-L) + S(k+1-l) + S(l))$$ By our hypothesis and simplifications we get that $S(k+1) = \frac{(k+1)k}{2}$ as intended. 
\end{proof}

\begin{tcolorbox}[drop shadow, title=(Useful analogy),lower separated=true]
An intuition for our above result is the handshaking example. Suppose that a group of $n$ people split up into a group of $n-k$ and $k$ people. Hence at every splitting there are $(n-k)k$ handshakes. And at the end there are obviously $n-1 + n-2 + \ldots + 1$ many handshakes which is equal to $\frac{n(n-1)}{2}(by summation for arithmetic series)$
\end{tcolorbox}
