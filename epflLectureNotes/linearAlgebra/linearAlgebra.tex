...
\begin{align*}
    v_{1} = x_{1}\\
    v_{2} = x_{2} - \frac{x_{2}\cdot v_{1}}{v_{1}^2}\\
    \ \vdots
\end{align*}
...
\begin{thm}
If is an $m\cross n$ matrix with \li \ columns we have that $A = QR$ where columns of $Q$ are an orthonormal basis for $Col(A)$ and $R$ is upper triangular. 
\end{thm}
